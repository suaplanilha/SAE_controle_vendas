...
function normalizedCatalogName_(value) {
  return String(value || '').trim().toLowerCase().normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ');
}
function saveFinanceCatalog(input){return safeCall_(function(){input=input||{};const entity=String(input.entity||''),name=String(input.nome||'').trim(),now=new Date();if(!name)return fail_('VALIDATION_ERROR','Informe o nome.');const lock=LockService.getScriptLock();lock.waitLock(20000);try{const all=readFinanceCatalog_();let list,sheet,headers,row,scope;if(entity==='CATEGORY'){list=all.categories;sheet=finSheet_(FIN.CATS,FIN.CAT_H);const fonte=String(input.fonte||'');if(!['RECEITA','DESPESA'].includes(fonte))return fail_('VALIDATION_ERROR','Selecione a fonte.');const tipo=fonte==='DESPESA'?String(input.tipo_custo||''):'';if(fonte==='DESPESA'&&!['FIXO','VARIAVEL'].includes(tipo))return fail_('VALIDATION_ERROR','Selecione Fixo ou Variável.');scope=x=>x.fonte===fonte&&x.tipo_custo===tipo;row=[input.uuid||Utilities.getUuid(),now,fonte,tipo,name,input.ativo!==false,now];headers=FIN.CAT_H;}else if(entity==='SUBCATEGORY'){list=all.subcategories;sheet=finSheet_(FIN.SUBS,FIN.SUB_H);if(!all.categories.some(x=>x.uuid===input.parent_uuid))return fail_('VALIDATION_ERROR','Categoria inválida.');scope=x=>x.categoria_uuid===input.parent_uuid;row=[input.uuid||Utilities.getUuid(),now,input.parent_uuid,name,input.ativo!==false,now];headers=FIN.SUB_H;}else if(entity==='ITEM'){list=all.items;sheet=finSheet_(FIN.ITEMS,FIN.ITEM_H);if(!all.subcategories.some(x=>x.uuid===input.parent_uuid))return fail_('VALIDATION_ERROR','Subcategoria inválida.');scope=x=>x.subcategoria_uuid===input.parent_uuid;row=[input.uuid||Utilities.getUuid(),now,input.parent_uuid,name,String(input.descricao||''),input.ativo!==false,now];headers=FIN.ITEM_H;}else return fail_('VALIDATION_ERROR','Entidade inválida.');const current=list.find(x=>x.uuid===String(input.uuid||''));if(list.some(x=>x.uuid!==String(input.uuid||'')&&scope(x)&&normalizedCatalogName_(x.nome)===normalizedCatalogName_(name)))return fail_('DUPLICATE_NAME','Já existe um cadastro com este nome neste nível.');if(current){row[1]=new Date(current.data_cadastro);sheet.getRange(current._row,1,1,headers.length).setValues([row]);}else sheet.appendRow(row);return ok_({uuid:row[0]});}finally{lock.releaseLock();}});}
...
function financeSnapshot_(input,catalog){const cat=catalog.categories.find(x=>x.uuid===input.categoria_uuid&&x.ativo),sub=catalog.subcategories.find(x=>x.uuid===input.subcategoria_uuid&&x.ativo&&x.categoria_uuid===input.categoria_uuid),item=input.item_uuid?catalog.items.find(x=>x.uuid===input.item_uuid&&x.ativo&&x.subcategoria_uuid===input.subcategoria_uuid):null;if(!cat||!sub||(input.item_uuid&&!item))throw new Error('Categoria ou subcategoria inválida/inativa.');if(cat.fonte!==input.fonte)throw new Error('Categoria incompatível com a fonte.');return{cat,sub,item:item||{uuid:'',nome:sub.nome}};}
